...
const helpSections: Record<string, { title: string; content: string }> = {
  ...
  settings: {
    ...
    content: [
      ...
      '`/settings section: incidents` - Configure incident channel category and ticket access roles',
  ...
  incidents: {
    ...
    content: [
      ...
      '2. Optional: set `incident-reminder-interval` in `/settings` and `incidents-category` plus `ticket-access-roles` in `/settings section: incidents`.',
